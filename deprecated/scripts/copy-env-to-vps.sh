...
# VPS için .env dosyasını tüm paketlere kopyalar
# Kullanım: ./scripts/copy-env-to-vps.sh
...
echo "🔍 VPS .env dosyası aranıyor: $ENV_FILE"

if [ ! -f "$ENV_FILE" ]; then
  echo "❌ HATA: $ENV_FILE bulunamadı!"
  echo "📝 Lütfen önce root dizinde env.vps dosyası oluşturun."
  exit 1
fi

echo "✅ VPS .env dosyası bulundu"
...
echo "📦 Paketlere VPS .env dosyası kopyalanıyor..."
...
echo ""
echo "🎉 Tüm paketlere VPS .env dosyası başarıyla kopyalandı!"
...
echo "🚀 VPS'de PM2 ile başlatmak için: pm2 start ecosystem.vps.js" 