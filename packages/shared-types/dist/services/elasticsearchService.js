...
class ElasticsearchService {
    constructor(node = process.env.ELASTICSEARCH_URL || 'http://localhost:9200', indexName = process.env.ELASTICSEARCH_INDEX || 'benalsam_listings') {
        ...
        try {
            ...
            if (response.suggest && response.suggest['listing-suggest'] && response.suggest['listing-suggest'][0]) {
                ...
                if (Array.isArray(listingSuggest.options)) {
                    suggestions.listings = listingSuggest.options.map((opt) => ({
                        text: opt.text,
                        score: opt.score,
                        category: 'listing'
                    }));
            ...
            if (response.suggest && response.suggest['category-suggest'] && response.suggest['category-suggest'][0]) {
                ...
                if (Array.isArray(categorySuggest.options)) {
                    suggestions.categories = categorySuggest.options.map((opt) => ({
                        text: opt.text,
                        score: opt.score,
                        category: 'category'
                    }));
     ...
     */
    addFilters(filterArray, filters) {
        if (filters.category) {
        ...
        if (filters.budget) {
            ...
            if (filters.budget.min !== undefined)
                range.budget.gte = filters.budget.min;
            if (filters.budget.max !== undefined)
                range.budget.lte = filters.budget.max;
     ...
     */
    buildAggregations(aggregationNames) {
        const aggs = {};
