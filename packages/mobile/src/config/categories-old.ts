import { Baby, Bitcoin, Book, Briefcase, Building, Camera, Car, Dumbbell, Gamepad2, GraduationCap, Home, Laptop, Music, Palette, Plane, Shirt, Smartphone, Utensils, Wrench } from "lucide-react-native";
...
const getCategoryPath = (categoryName: string, subCategoryName?: string, subSubCategoryName?: string) => {
...
const findCategoryByName = (name: string) => {
    ...
    let currentLevel: any = categoriesConfig;
    let foundCategory = null;

    for (const part of pathParts) {
        const category = currentLevel.find((cat: any) => cat.name === part);
        if (category) {
