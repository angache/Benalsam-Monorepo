...
serve(async (req) => {
  ...
  try {
    ...
    // Get request body
    const { userId, token, notification, type = 'single' }: SendNotificationRequest = await req.json()
    ...
    if (tokens.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No tokens found for notification' }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (!expoResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to send notification via Expo' }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
